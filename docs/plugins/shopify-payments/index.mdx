...
This document outlines the steps to install, configure, and test the Credova Payments Shopify app. It includes details on accessing the Credova Payments Portal, retrieving API keys, and enabling the payment provider in Shopify.
...
## Step 1: Install the Credova Payments App

1. Go to Credova Payments on Shopify - https://apps.shopify.com/publicsquare-pay
...
2. Copy and paste your `Publishable Key` and `Secret Key` from the Credova Portal developers page into the `Publishable Key` and `Secret Key` fields.
...
<Alert>
**Test cards for Credova Payments**

Available test cards: https://developers.publicsquare.com/api/testing#cards

Use the test card details provided in the Credova Developers docs to simulate succeeded and failed transactions.
</Alert>
