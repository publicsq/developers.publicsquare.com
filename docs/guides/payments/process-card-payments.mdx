...
<Alert type="warning">When you receive a `202`, subscribe to the `payment-intents:update` webhook event. The final outcome is in `entity.status` on the Payment Intent in the notification payload. See the [Webhooks](/concepts/webhooks) guide for setup instructions.</Alert>

```json showLineNumbers title="Webhook event: payment-intents:update"
{
...
The direct Payments API provides a synchronous, single-request charge. The response contains the final payment result. It remains supported for existing
