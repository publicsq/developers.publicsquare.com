...
Now that you have completed the [Collect Apple Pay](/guides/payment-methods/collect-apple-pay) guide, let's process an Apple Pay payment with the newly captured Apple Pay.
...
We will need to use the information from the [previously collected Apple Pay](/guides/payment-methods/collect-apple-pay) to create a new payment.
...
If you [attached the Apple Pay to a customer](/api/financial/attach-apple-pay-to-customer), then you can omit the `customer` section when processing the payment:
...
If you supplied the `billing_details` or [updated the Apple Pay](/api/financial/update-apple-pay-payment-method), then you can omit the `billing_details` section when processing the payment:
...
Now that we have seen how to process payments with a collected Apple Pay, we may need to manage the payment or view information about the payment.
