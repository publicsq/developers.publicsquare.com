...
<Alert>
  Card verification is the one flow that still uses the direct Payments API: a [Payment Intent](/concepts/payment-intents) requires an amount greater than
  zero. Use payment intents for anything you intend to charge.
</Alert>
