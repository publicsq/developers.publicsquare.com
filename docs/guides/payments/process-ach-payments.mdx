...
<Alert>
  New integrations should use [Payment Intents](/concepts/payment-intents), the recommended way to collect a payment. An intent accepts a `bank_account`
...
We will need to use the information from the previously [manually collected bank account](/guides/payment-methods/collect-bank-accounts) or [collected verified bank account](/guides/payment-methods/collect-verified-bank-accounts) to create a new payment.
