...
<Alert>This behavior requires the latest release of the Credova Elements SDKs. See the [Migration Notes](#migration-notes) below if you're upgrading from an older version.</Alert>
...
Google Pay collects the billing address in its `FULL` format, meaning customers see and enter a complete street address (not just a postal code); a billing phone number is not requested, so it won't appear on the Google Pay sheet.
...
async function onPaymentAuthorized(paymentData) {
  ...
  // highlight-start
  // Map the shipping address into the shipping_address field of the create-payment request
...
If you're upgrading an existing Google Pay integration, note the following breaking change:

- `billing_details` has been removed from the Google Pay `create()` input on both the JavaScript and React SDKs. It is no longer accepted by [Create a Google Pay Token](/api/financial/create-google-pay-token) (`POST /payment-methods/google-pay`) &mdash; if you still send it, the field is silently ignored. The payment method's `billing_details` now always comes from the address collected by the Google Pay sheet, as described in [Billing and Shipping Addresses](#billing-and-shipping-addresses).
