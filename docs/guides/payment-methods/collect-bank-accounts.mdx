...
# Collect Bank Accounts Manually

In this guide, we will set up Credova Elements SDKs to capture bank accounts in a frontend application and securely authorize and store the bank account data while avoiding PCI requirements.
...
First, you will need to request access to the `payment-methods:bank-accounts:manual-creation` account setting type.

To do this, go to the [merchant portal](https://portal.publicsquare.com/settings/features) and click on the `Request this feature` button next to the `Manual Bank Account Creation` setting.
Alternatively, you can request access via the [API](/api/accounts/request-account-setting-access).

This is required to enable manual bank account creation for your account.
