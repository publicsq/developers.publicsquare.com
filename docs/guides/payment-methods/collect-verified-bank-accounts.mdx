...
Now that you verified bank account data in your web application, it is time to store the bank account in your account.

To do this, we will call the `create bank account` method from the SDK passing the `bank_account_verification_id`, returned from the bank account verification result, as data points in the payload.
