...
Every payout type also needs an onboarded account and a funded holding account. Payouts draw from your holding account balance, so check the balance with the
[Get holding account balance](/api/accounts/get-holding-account-balance-by-id) endpoint. A payout is rejected with `holding account does not have sufficient
funds` when the balance is too low, and with `account has not finished onboarding for payouts.` when your account cannot send funds yet.
