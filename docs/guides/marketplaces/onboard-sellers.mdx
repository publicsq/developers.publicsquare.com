...
This will return a response containing an iFrame URL you can mount on your site to prompt the seller to provide their businss bank account:
    ...
    const handleMessage = async (e) => {
        ...
        if (data && typeof data === 'object') {
            if (data.step === 'APP_MOUNTED') {
                setIsLoading(false);
            } else if (data.step === 'REDIRECT' && data.loginId) {
...
-d '{
  ...
  "requeset_id": "a5b739e0-9448-484d-b553-77d7fb8567e0"
}'
```
...
PublicSquare provides an [onboarding review endpoint](/api/accounts/review-seller-onboarding-by-account-id) to be able to check the status of your onboarding requirements and submission status:
...
You can receive automnated notifications as your seller's onboarding status changes by [creating a webhook](/concepts/webhooks) and subscribing to the `onboarding:seller:update` event type.
