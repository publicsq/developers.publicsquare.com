...
As a marketplace, you will need to create and onboard your sellers to be able to transfer funds to them. Every seller within your marketplace will need a seller account created with Credova.
...
## Authenticating as a Seller Account
All seller API requests must include the <code>X-ACCOUNT-ID</code> header set to the seller's account ID. For example:

<code>X-ACCOUNT-ID: acc_w6dogDaHuU6h1N5e5vfXLUYf</code>

## Save Business Details

Referencing the `id` of the previously created seller account, we can now save business details for our seller account.

Credova exposes several endpoints to enable you to submit all of the required business details:

* [Save Business Details](/api/accounts/save-business)
* [Save Business Representative](/api/accounts/save-business-representative)
* [Save Business Tax Information](/api/accounts/save-tax-information)
* [Create Ultimate Beneficial Owner](/api/accounts/create-ultimate-beneficial-owner)
* [Upload Business Document](/api/accounts/upload-business-document)
...
```shell showLineNumbers title="Get Bank Account Verification iFrame URL"
curl -L 'https://api.publicsquare.com/accounts/acc_w6dogDaHuU6h1N5e5vfXLUYf/business/bank-account/verification' \
-H 'Accept: application/json' \
-H 'X-API-KEY: <SECRET_API_KEY>' \
-H 'X-ACCOUNT-ID: acc_w6dogDaHuU6h1N5e5vfXLUYf'
```
...
Using the code above, your API should receive a `verification_code`, `request_id` and `bank_account_id` which we can then [trigger the bank verification process](/api/accounts/save-bank-verification):
...
Once we have the `bank_account_verification_id`, we can then [save the bank account](/api/accounts/save-bank-account) for the seller:
...
```
<Alert>If Credova has not yet received a response back from Flinks, we will return a `202 - Accepted` HTTP status code. Please retry the request.</Alert>
...
Credova provides an [onboarding review endpoint](/api/accounts/review-onboarding) to be able to check the status of your onboarding requirements and submission status:
...
Now that all of your requirements are `complete`, we can [submit the seller for underwriting](/api/accounts/onboard):
