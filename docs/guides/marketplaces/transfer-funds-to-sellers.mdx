...
Credova sends a `transfer_intent:update` [webhook](/concepts/webhooks) on every status transition. The `entity.status` field in the webhook payload reflects the final state — use it to confirm completion regardless of what the `confirm` response returned.
