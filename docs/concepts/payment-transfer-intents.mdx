...
flowchart TB
    ...
    PROCESSING -- "transfer settles" --> SUCCEEDED["SUCCEEDED"]
    PROCESSING -- "transfer fails" --> ERROR
    SUCCEEDED --> End(["End"])
    CANCELLED --> End
    ERROR --> End
...
| `processing` | `confirm` succeeded and the transfer has been submitted for settlement. The intent remains in this state until settlement completes or fails.            |
| `succeeded`  | Settlement completed and funds are en route to the seller's account. Terminal state.                                                                     |
...
- Submission for settlement failed during `confirm`.
- Settlement did not complete within 1 hour of submission.
...
| `confirm` accepted but settlement result not yet received | `202 Accepted`                                            |

<Alert>A `202 Accepted` response from `confirm` means the transfer has been submitted for settlement but a final result has not yet been received. The intent will move to `processing`, and its terminal state will be reported via webhook once settlement completes.</Alert>
