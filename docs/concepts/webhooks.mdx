...
| `payment:update`             | This event publishes the [Payment](/api/financial/get-payment-by-id) entity when a payment is created or updated. This also fires when a capture is created and whenever a capture's status changes (for example, pending to succeeded); check the `payment_captures` array along with `status`, `amount_charged`, and `amount_capturable` on the payload.                                                                        |
| `payment_intent:update`     | This event publishes the [Payment Intent](/api/financial/get-payment-intent-by-id) entity when a payment intent is created or updated.                                                     |
...
| `transfer_intent:update`     | This event publishes the [Payment Transfer Intent](/concepts/payment-transfer-intents) entity when a transfer intent is created or its status changes.                                    |
...
Webhook Events can be viewed from the [Webhook page](https://portal.publicsquare.com/developers/webhooks) in the Credova Portal or cam be [retrieved from the API](/api/accounts/get-all-webhook-events) for the Webhook.
