...
flowchart TB
    ...
    RCAP -- "partial capture (is_final: false)" --> RCAP
...
| `requires_capture` | The card was authorized with `capture_method: manual`. The authorization holds funds for up to 7 days. Call [Capture Payment Intent](/api/financial/capture-payment-intent) to complete the charge. With partial captures, the intent stays `requires_capture` while the underlying payment reports `partially_captured`, until a final capture or a release closes the authorization. See [Multiple partial captures](/guides/payments/authorize-and-capture-payments#multiple-partial-captures). |
