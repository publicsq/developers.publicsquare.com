...
| `transfer`   | Transfers are used to send funds to [connected sellers](/concepts/connections) when creating a payment. For instance, if you charge a customer $300, you can transfer $100 to three different sellers as part of the same payment. |
...
| `error`            | The payment or refund attempt failed due to an unexpected system or integration issue, such as an invalid response from the processor, network errors, or a missing required parameter. This status applies to all payment types.|
